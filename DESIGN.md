...
1. **Memory.** The walker now streams file paths through a buffered channel (`walkBufSize = 64`) rather than accumulating the full path list before returning. `cmd/root.go` fans each path out to per-detector channels as it arrives, so peak memory for the path list is bounded by the channel buffer depth rather than the total file count. The remaining concern at extreme scale is that each detector still accumulates its own `[]string` before calling `Detect` — a fully streaming detector interface would eliminate this too, but requires changing the `Detector` contract.
